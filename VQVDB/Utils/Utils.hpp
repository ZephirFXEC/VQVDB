...
class VdbPrimCIterator {
   public:
	...
	explicit VdbPrimCIterator(const GEO_Detail* gdp, const GA_PrimitiveGroup* group = nullptr, FilterFunc filter = FilterFunc());
...
class VdbPrimIterator : public VdbPrimCIterator {
   public:
	...
	///     to a GU_PrimVDB and returns a boolean specifying whether
	///     that primitive should be visited (@c true) or not (@c false)
	explicit VdbPrimIterator(const GEO_Detail* gdp, const GA_PrimitiveGroup* group = nullptr, const FilterFunc& filter = FilterFunc())
...
class HoudiniInterrupter final : public openvdb::util::NullInterrupter {
   public:
	explicit HoudiniInterrupter(const char* title = nullptr) : mUTI{UTgetInterrupt()}, mRunning{false}, mTitle{title ? title : ""} {}
...
using ScalarGridTypes =
    openvdb::TypeList<openvdb::BoolGrid, openvdb::FloatGrid, openvdb::DoubleGrid, openvdb::Int32Grid, openvdb::Int64Grid>;
	...
	if (vdb.hasGrid()) {
		...
		if (makeUnique) {
			...
			if (treePtr.use_count() > 2) {  // grid + treePtr = 2
				...
				gridPtr->apply<GridTypeListT>([](Grid& baseGrid) { baseGrid.setTree(baseGrid.constBaseTree().copy()); });
			}
		}
		return gridPtr->apply<GridTypeListT>(op);
	}
...
template <typename GridT>
UT_ErrorSeverity loadGrid(const GU_Detail* aGeo, typename GridT::Ptr& grid, const UT_StringHolder& group) {
	const GA_PrimitiveGroup* groupRef = aGeo->findPrimitiveGroup(group);
	for (openvdb_houdini::VdbPrimIterator it(aGeo, groupRef); it; ++it) {
